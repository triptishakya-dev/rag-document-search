...
-   **📄 Intelligent Ingestion**: Securely upload PDFs with automated text extraction and high-quality chunking.
-   **⚡ Async Processing**: Redis-powered task queue (BullMQ) handles PDF parsing and embedding without blocking the API.
-   **🔍 Semantic Search**: Vector embeddings stored in **Qdrant DB** enable precise retrieval of relevant context.
-   **💬 AI-Powered Chat**: Grounded responses generated by **Google Gemini 2.5 Flash**, including source citations.
-   **📦 Cloud Storage**: Reliable file persistence using **Amazon S3**.
-   **🗄️ Structured Persistence**: Metadata and chunk tracking managed via **PostgreSQL** & **Prisma ORM**.
...
```
├── backend
│   ├── controllers     # API business logic (chat, upload)
│   ├── lib             # Core services (S3, Embedding, Queue)
│   ├── routes          # Express route definitions
│   ├── index.js        # API Entry point
│   ├── worker.js       # Background processing worker
│   └── prisma          # DB Schema & Migrations
├── frontend
│   ├── app             # App Router pages (Chat, Upload, How-it-works)
